<?php
	class Email {
		...
		public static function enviar( $body, $nome, $email ,$getid ,$formulario){
			$retorno = 0;
			...
			if( $formulario == "Recuperar Senha"){
				$mail->AddAddress($email, $nome);
			}else{
				$mail->AddAddress('sac@tecnocalconstrutora.com.br', 'SAC');
				$mail->AddCC('wanessa@tecnocalconstrutora.com.br', 'Wanessa');				
			}
			 //E-mail do desenvolvedor (para testes)
				$mail->AddBCC('bcc@summercomunicacao.com.br', 'BCC Summer');

			// Define o retorno de resposta
				$mail->addReplyTo('wanessa@tecnocalconstrutora.com.br');
			...
			$mail->Subject = $tipo . ' | Nº ' . $getid . ' - '.$formulario.' Tecnocal'; // Assunto da mensagem
